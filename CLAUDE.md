...
6. `memory/pead_health.md` — weekly PEAD signal-health gate (RISK_ON/RISK_OFF). Read by `pre_market` as the AND gate on new entries. Written only by `universe_refresh` (via `compute_pead_health.py`). Read-only to every other routine.
