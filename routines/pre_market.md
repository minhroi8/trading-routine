...
4c. **PEAD signal-health AND gate (new buys only).** Read `memory/pead_health.md` frontmatter.
   - **Freshness:** if `expires_on` is in the past, or the file is missing/placeholder (`source: unset`), treat the gate as **RISK_ON but flagged STALE** — do not block on the health file alone (the universe-cache freshness check above is the hard gate that halts the run). Note the staleness in `research_log.md` and the Discord message.
   - **If `gate: RISK_OFF`:** do NOT plan any new buys today. The post-earnings-drift edge is currently off (either SPY < 200MA, or trailing realized PEAD health < 0 — bull-and-weak). Still plan any required SELLS (exits are never gated). Write `plan.md` with zero buys and a Notes line quoting `gate`, `spy_above_200ma`, `realized_health_60d_pct`, and `health_sample_n`.
   - **If `gate: RISK_ON`:** proceed normally to step 5.
   - This gate governs entries only; it does not override any `strategy.md` rule and never affects exit logic.
5. **Draft `memory/plan.md`** for each surviving candidate (skip the buy block entirely if step 4c is RISK_OFF):
...
- Recompute or hand-edit `memory/pead_health.md` — it is read-only here, written only by `universe_refresh`.
- Plan any NEW buy when the PEAD health gate is `RISK_OFF` (step 4c). Exits are exempt.
- Leave `plan.md` blank if you had candidates — if nothing qualified (including a RISK_OFF gate), write that reason explicitly under Notes.
